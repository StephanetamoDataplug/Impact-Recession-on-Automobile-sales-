...
def update_output_container(report_type, year):
    if report_type == 'Recession Period Statistics':
        ...
        R_chart2 = dcc.Graph(
            figure=px.bar(average_sales, x='Vehicle_Type', y='Automobile_Sales', title="Average Vehicle Sales during Recession Period")
        )

        # Plot 3: Pie chart for total expenditure share by vehicle type during recessions
        exp_rec = recession_data.groupby('Vehicle_Type')['Advertising_Expenditure'].sum().reset_index()
        R_chart3 = dcc.Graph(
            figure=px.pie(exp_rec, values='Advertising_Expenditure', names='Vehicle_Type', title="Advertisement Expenditure on Vehicle Type During Recession")
        )
    ...
    elif report_type == 'Yearly Statistics' and year is not None:
